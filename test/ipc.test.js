...
test('every channel a preload or main module uses is listed, and every listed one is used', () => {
  ...
  const files = [
    ...fs
      .readdirSync(path.join(dir, 'renderer'))
      .filter((f) => f.endsWith('-preload.js'))
      .map((f) => path.join(dir, 'renderer', f)),
    ...fs.readdirSync(path.join(dir, 'main')).map((f) => path.join(dir, 'main', f)),
  ];
  const used = new Set();
  for (const f of files)
    for (const name of literals(f)) {
      assert.ok(known.has(name), `${path.basename(f)} uses unknown channel ${name}`);
      used.add(name);
    }
