...
test('formatLine: one JSON line with time, level, message and data; errors flattened', () => {
  const line = formatLine({ level: 'warn', msg: 'avatar', data: { code: 7 }, now: 1_700_000_000_000 });
  assert.deepEqual(JSON.parse(line), { t: '2023-11-14T22:13:20.000Z', level: 'warn', msg: 'avatar', code: 7 });
  assert.ok(line.endsWith('\n'));
  const err = JSON.parse(formatLine({ level: 'error', msg: 'boom', data: { err: new TypeError('bad') }, now: 0 })).err;
