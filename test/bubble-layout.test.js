...
const { bubbleLayout, stackBounds, resizeAnchor, edgeOf, BASE, BANNER } = require('../src/lib/bubble-layout');
...
test('a lone disc on the right: the window spans the banner toward the centre, the page places the disc at its far side', () => {
  ...
  assert.deepEqual(l.window, { x: right.x + 44 - BANNER - PAD, y: right.y - PAD, width: BANNER + 2 * PAD, height: 44 + 2 * PAD });
  assert.equal(l.renderer.edge, 'right');
  assert.equal(l.renderer.contentX, PAD + BANNER - 44);
  assert.equal(l.renderer.contentY, PAD);
  assert.deepEqual([l.renderer.base, l.renderer.banner, l.renderer.pitch], [BASE, BANNER, FAN_ITEM]);
});
...
test('the banner\'s extra rows go above the disc when there is room, and cross over in page px', () => {
  const l = bubbleLayout({ anchor: right, size: 66, scale: 1.5, fanCount: 0, bannerExtra: 60, area });
  assert.equal(l.direction, 'up');
  assert.equal(l.window.height, 66 + 90 + 2 * PAD * 1.5);
  assert.equal(l.renderer.contentY, (PAD * 1.5 + 90) / 1.5);
  const top = bubbleLayout({ anchor: { x: 10, y: 10 }, size: 44, scale: 1, fanCount: 0, bannerExtra: 60, area });
...
test('stackBounds is the disc alone, or the column beside which the panel sits', () => {
  assert.deepEqual(stackBounds({ anchor: right, size: 44, scale: 1, fanCount: 0, bannerExtra: 0, area }), { ...right, width: 44, height: 44 });
  const s = stackBounds({ anchor: right, size: 44, scale: 1, fanCount: 2, bannerExtra: 0, area });
  assert.deepEqual(s, { x: right.x, y: right.y - 2 * FAN_ITEM, width: 44, height: 44 + 2 * FAN_ITEM });
});
