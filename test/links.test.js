...
test('matchesUrlPattern understands scheme, host and path wildcards', () => {
  assert.equal(
    matchesUrlPattern('wss://edge-chat.messenger.com/chat', 'wss://edge-chat.messenger.com/*'),
    true,
  );
  assert.equal(
  ...
  assert.equal(
    matchesUrlPattern('wss://edge-chat.messenger.com/', 'https://*.messenger.com/*'),
    false,
  );
  assert.equal(matchesUrlPattern('garbage', 'https://*.facebook.com/*'), false);
});
