...
const { initial, reduce, decide, SOCKET_GRACE_MS, STALE_MS, BACKOFF_MS } = require('../src/lib/liveness');
...
test('a page just loaded, with traffic, never reloads', () => {
  const s = play([['loaded', 0], ['socket-open', 1000], ['request-ok', 5 * MIN]]);
...
test('waking from sleep reloads once, when online, and never while the panel shows', () => {
  let s = play([['loaded', 0], ['resume', 60 * MIN]]);
  assert.deepEqual(decide(s, { visible: true, online: true, now: 60 * MIN + 1 }), { reload: false, reason: 'visible' });
  assert.deepEqual(decide(s, { visible: false, online: false, now: 60 * MIN + 1 }), { reload: false, reason: 'offline' });
...
test('a connection that errored and did not come back within the grace period reloads', () => {
  let s = play([['loaded', 0], ['socket-open', 1000], ['request-ok', 9 * MIN], ['socket-error', 10 * MIN]]);
  assert.equal(decide(s, { ...up, now: 10 * MIN + SOCKET_GRACE_MS - 1 }).reload, false);
  assert.deepEqual(decide(s, { ...up, now: 10 * MIN + SOCKET_GRACE_MS }), { reload: true, reason: 'socket-error' });
...
test('no completed request for a long while, online, means a stall', () => {
  const s = play([['loaded', 0], ['request-ok', 2 * MIN]]);
  assert.equal(decide(s, { ...up, now: 2 * MIN + STALE_MS - 1 }).reload, false);
  assert.deepEqual(decide(s, { ...up, now: 2 * MIN + STALE_MS }), { reload: true, reason: 'stale' });
