...
test('keeps well-formed rows in order, capped at 5', () => {
  ...
  assert.deepEqual(
    normalizeRows(rows).map((r) => r.href),
    ['/t/1/', '/t/2/', '/t/3/', '/t/4/', '/t/5/'],
  );
});
...
test('dedupes by href and coerces fields', () => {
  ...
  assert.deepEqual(out, [
    { href: '/t/1/', name: 'A', avatarUrl: null, unread: true, preview: '', time: '' },
  ]);
});
...
test('spanText restores sprite emoji into the preview', () => {
  const span = el('SPAN', [
    text('Kim: '),
    el('IMG', [], { src: 'https://static.xx.fbcdn.net/images/emoji.php/v9/x.png', alt: '😢' }),
  ]);
...
test('spanText ignores images that are not emoji sprites', () => {
  const span = el('SPAN', [
    text('Kim'),
    el('IMG', [], { src: 'https://cdn/avatar.jpg', alt: 'Kim Lee' }),
  ]);
  assert.equal(spanText(span), 'Kim');
...
const box = (h, parent, scrollTop = 0) => ({
  scrollHeight: h,
  clientHeight: 500,
  scrollTop,
