...
const preload = fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer', 'panel-preload.js'), 'utf8');
...
test('the panel preload carries the row reader verbatim', () => {
  for (const fn of [spanText, listAtTop, readRows]) assert.ok(preload.includes(fn.toString()), `${fn.name} differs from lib`);
});
...
test('the panel preload exposes nothing to the page', () => {
  ...
  assert.equal(/contextBridge|exposeInMainWorld|window\.\w+\s*=|\beval\(|new Function|executeJavaScript/.test(code), false);
