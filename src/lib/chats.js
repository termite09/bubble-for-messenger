...
const fullKey = (rows) => JSON.stringify(rows.map((r) => [r.href, r.name, r.unread, r.preview, r.time, Boolean(r.avatar)]));
const displayKeyOf = (rows) => JSON.stringify(rows.map((r) => [r.href, r.name, r.unread, Boolean(r.avatar)]));
...
function reduceRecent(state, rows, { visible }) {
  ...
  const landed = state.seeded && !visible ? rows.find((r, i) => {
...
const closeChat = (state, now) => (state.activeHref ? { ...state, activeHref: null, lastChat: { href: state.activeHref, closedAt: now } } : state);

// A click on the disc with nothing open: the remembered chat, or the stack.
const discClick = (state, now, seconds) => (reopenOpen(state.lastChat, now, seconds) ? { action: 'reopen', href: state.lastChat.href } : { action: 'stack' });
