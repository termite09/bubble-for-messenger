...
// own (Messenger's "You: …", or a thread bolded for another reason),
...
// landed last and is still unread (someone else's, never the user's own); before anything has
// landed, the top unread chat. Returns { href } or null.
function pickUnread({ recent, landed }) {
  const received = (r) => Boolean(r && r.unread && !OWN_MESSAGE.test(r.preview));
  if (landed && received(recent.find((r) => r.href === landed.href))) return { href: landed.href };
  const top = recent.find(received);
  return top ? { href: top.href } : null;
}
