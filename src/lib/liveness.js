...
const initial = (now) => ({
  loadedAt: now, lastRequestAt: now, socketErrorAt: null, resumePending: false,
  failLoadAt: null, failCount: 0, lastReloadAt: null, reloadCount: 0,
});

// Events: loaded, fail-load, socket-open, socket-error, request-ok, resume, reload (we did).
function reduce(state, event, now) {
  switch (event) {
    case 'loaded': return { ...state, loadedAt: now, lastRequestAt: now, failLoadAt: null, resumePending: false, socketErrorAt: null };
    case 'fail-load': return { ...state, failLoadAt: now, failCount: state.failCount + 1 };
    case 'socket-open': return { ...alive(state, now), socketErrorAt: null, lastRequestAt: now };
    case 'socket-error': return { ...state, socketErrorAt: state.socketErrorAt || now };
    case 'request-ok': return { ...alive(state, now), lastRequestAt: now };
    case 'resume': return { ...state, resumePending: true };
    case 'reload': return { ...state, lastReloadAt: now, reloadCount: state.reloadCount + 1, resumePending: false, socketErrorAt: null, failLoadAt: null, failCount: state.failCount };
    default: return state;
  }
}

// Traffic well after a reload means the reload worked: the next one need not wait long.
const alive = (state, now) => (state.lastReloadAt !== null && now - state.lastReloadAt >= ALIVE_AFTER_MS ? { ...state, reloadCount: 0 } : state);
...
function decide(state, { visible, online, now }) {
  ...
  const backoff = state.lastReloadAt === null ? 0 : BACKOFF_MS[Math.min(state.reloadCount - 1, BACKOFF_MS.length - 1)];
  if (state.lastReloadAt !== null && now - state.lastReloadAt < backoff) return none('backoff');
  if (state.failLoadAt !== null) return now - state.failLoadAt >= errorRetryDelay(state.failCount - 1) ? { reload: true, reason: 'fail-load' } : none('fail-wait');
  if (state.resumePending) return { reload: true, reason: 'resume' };
  if (state.socketErrorAt !== null && now - state.socketErrorAt >= SOCKET_GRACE_MS) return { reload: true, reason: 'socket-error' };
  if (now - Math.max(state.lastRequestAt, state.loadedAt) >= STALE_MS) return { reload: true, reason: 'stale' };
  return none(null);
}
