...
function press({ cursor, anchor, expanded }) {
  return { origin: cursor, offsetX: cursor.x - anchor.x, offsetY: cursor.y - anchor.y, collapsedOnPress: expanded, moved: false, hot: false };
}
...
function move(drag, { cursor, anchor, size, area, overDismiss = () => false }) {
  if (!drag.moved && isClick(cursor.x - drag.origin.x, cursor.y - drag.origin.y)) return { drag, position: null, becameDrag: false, hotChanged: false };
  const becameDrag = !drag.moved;
  const at = clampToArea({ x: cursor.x - drag.offsetX, y: cursor.y - drag.offsetY, width: size, height: size }, area);
  const position = at.x !== anchor.x || at.y !== anchor.y ? at : null;
  const hot = position ? Boolean(overDismiss(position)) : drag.hot;
  return { drag: { ...drag, moved: true, hot }, position, becameDrag, hotChanged: hot !== drag.hot };
}
