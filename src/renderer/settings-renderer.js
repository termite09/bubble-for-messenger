...
function render(s) {
  ...
  for (const input of document.querySelectorAll('input[type="radio"]:not([name="tab"])')) input.checked = input.value === s[input.name];
}
...
document.addEventListener('change', (e) => {
  ...
  else if (input.type === 'radio') window.settingsApi.set(input.name, input.value);
});
